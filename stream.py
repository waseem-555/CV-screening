...
# Function to convert PDF to image (first page for now)
def pdf_to_image(pdf_path):
    doc = fitz.open(pdf_path)
    images = []
    for page_num in range(min(1, len(doc))):  # Limit to 1 page for preview
        pix = doc[page_num].get_pixmap()
        img = pix.tobytes("png")  # Convert to PNG format
        images.append(img)
    return images
...
if uploaded_files:
    ...
    if query:
        ...
        for i, (cv_name, score) in enumerate(top_3_cvs):
            ...
            for img in images:
                st.image(img, caption=f"Preview of {cv_name}", use_column_width=True)

            st.markdown(f"[Download CV](file://{os.path.abspath(pdf_path)})")  # Link to download CV
